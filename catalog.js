(function attachEstimateCatalog(root, factory) {
  ...
  const features = [
    ...
    f("staging", "infra", "検証環境", "本番へ出す前に商談相手と確認できる別環境を用意します。", h(4, 0, 0, 8, 8, 28, 16, 6), ["web-deploy"], ["環境"]),
    ...
    f("uat-support", "qa", "受入テスト支援", "商談相手の確認用シナリオ、説明、修正管理を行います。", h(12, 4, 8, 8, 4, 0, 40, 16), ["staging", "documentation-training"], ["受入"]),
  ...
  const assessedPriceByFeature = {
    ...
    "charts-ui": assessed("M", 70000, "情報表示・一覧", "一覧6万円より描画・軸・凡例対応が増えるためM下限。", "API基盤を別項目として加算し、グラフ表示固有の追加分に限定する。"),
    ...
    "business-rules": assessed("M", 100000, "検索・絞り込み", "標準的な業務ロジックの公開価格帯10〜20万円の下側。", "金額・期限・判定の実装と試験を対象にし、APIとデータ設計は別項目として加算する。"),
    "workflow-engine": assessed("M", 120000, "予約・注文フロー", "状態遷移は標準業務機能の12〜20万円帯。", "履歴・計算基盤を別項目として加算し、状態遷移管理そのものに限定する。"),
    "approval-flow": assessed("M", 150000, "複雑な権限管理", "差戻し・承認履歴を伴うため標準ワークフローより上。", "状態管理・権限・通知の依存価格と重ならない承認固有処理を計上する。"),
    "estimate-invoice": assessed("M", 180000, "決済機能", "税・値引き・明細計算は金額誤りリスクが高くM上位。", "PDF・マスタ・計算基盤を別項目として加算し、見積請求ロジックに価格を置く。"),
    "product-catalog": assessed("M", 100000, "検索・絞り込み", "商品CRUDは標準業務画面の10万円帯。", "管理画面とマスタ管理を別項目として加算し、商品・価格・公開状態の処理に限定する。"),
    inventory: assessed("L", 180000, "予約・注文フロー", "引当・棚卸・入出庫の整合性管理はL下限相当。", "商品台帳と履歴を別項目として加算し、在庫固有の整合性処理を評価する。"),
    ...
    "report-generator": assessed("M", 150000, "PDF出力", "帳票公開価格10万円〜に差込み・改ページ・表配置を加味。", "PDF変換単体を別項目として加算し、テンプレート生成固有の作業を計上する。"),
    "scheduled-jobs": assessed("M", 70000, "プッシュ通知", "単純バッチの公開価格15万円〜を、共通キューを別項目として加算することで抑制。", "スケジュール登録と実行管理に限定しM下限とする。"),
    "schema-design": assessed("S", 60000, "データベース連携", "多数機能から自動追加される共通設計のためS上限。", "DB環境と個別ロジックを別項目として加算し、テーブル・関係・制約設計だけを一度計上する。", { pricingClass: "foundation" }),
    "master-data": assessed("M", 80000, "管理画面", "選択肢・単価管理は標準CRUDの8〜12万円帯。", "フォームと管理画面を別項目として加算し、マスタ処理固有部分に限定する。"),
    "history-versioning": assessed("M", 100000, "セキュリティ対策", "差分保存・変更者・復元は標準データ機能より高い。", "認証とデータ設計を別項目として加算し、履歴・版管理固有の処理を評価する。"),
    "file-storage": assessed("M", 70000, "画像添付", "保管処理は添付UI10万円より軽くM下限。", "添付画面・公開環境・セキュリティを別項目として加算し、権限付き保管だけを計上する。", { pricingClass: "foundation" }),
    "search-index": assessed("M", 80000, "検索・絞り込み", "索引生成は検索機能10万円の基盤部分。", "検索画面を別項目として加算し、索引の作成・更新処理に限定する。"),
    "analytics-store": assessed("M", 90000, "データベース連携", "集計履歴と分析用構造は標準DB機能の中位。", "DB基盤・定期処理・表示画面を別項目として加算する。"),
    "backup-restore": assessed("M", 80000, "インフラ構築", "公開事業者の単純設定5.5万円と復元確認を考慮。", "DB・監視・DRを別項目として加算し、バックアップ設定と復元確認に限定する。", { pricingClass: "foundation" }),
    "data-migration": assessed("M", 160000, "CSVダウンロード", "検査・変換・欠損対応・照合が必要なため入出力8万円の2倍。", "標準的なデータ量を前提とし、件数・品質で別途補正する。"),
    "user-accounts": assessed("M", 70000, "会員登録・ログイン", "登録・停止・退会データ処理は認証12万円の基盤部分。", "ログインUIとセッションを別項目として加算し、アカウント管理だけを評価する。", { pricingClass: "foundation" }),
    "session-security": assessed("S", 40000, "会員登録・ログイン", "複数認証方式から共用されるためS帯。", "期限切れ・ログアウト・端末管理の共通部分だけを一度計上する。", { pricingClass: "foundation" }),
    mfa: assessed("M", 80000, "SNSログイン連携", "認証方式を1つ追加する既存単価8万円と同水準。", "通知とセッションを別項目として加算し、多要素認証固有部分を計上する。"),
    "organization-tenancy": assessed("L", 250000, "複雑な権限管理", "データ混在が重大事故になるためL中位。", "会社境界の設計・実装・境界試験へ重点配分する。"),
    "external-portal-access": assessed("M", 150000, "複雑な権限管理", "招待・期限・対象データ制限が必要なM上位。", "権限管理と監査ログを別項目として加算し、社外アクセス固有部分を評価する。"),
    "audit-log": assessed("M", 90000, "セキュリティ対策", "証跡保存・検索・改ざん防止を含む標準セキュリティ機能。", "複数機能から共用されるためM中位に抑え一度だけ計上する。", { pricingClass: "foundation" }),
    "security-review": assessed("M", 100000, "セキュリティ対策", "公開価格と専門レビュー工程の標準帯。", "基本設計と試験を別項目として加算し、第三者確認と是正に限定する。"),
    "file-upload": assessed("M", 70000, "画像添付", "汎用文書添付は画像プレビュー・並替えより軽い。", "保管とAPIを別項目として加算し、アップロードUIと失敗処理に限定する。"),
    "image-processing": assessed("S", 60000, "画像添付", "画像添付から自動追加される補助処理としてS上限。", "保管・非同期基盤を別項目として加算し、圧縮とサムネイル生成だけを評価する。"),
    "camera-capture": assessed("S", 60000, "画像添付", "カメラ起動と登録導線は画像添付の追加機能。", "画像添付とレスポンシブを別項目として加算し、撮影導線だけを計上する。"),
    "offline-sync": assessed("L", 300000, "外部システム多数連携", "競合解決・再送・端末保存・障害試験を伴う高難度L機能。", "通信不安定時のデータ損失リスクを価格へ反映する。"),
    webhooks: assessed("M", 70000, "外部システム多数連携", "単一連携の署名検証・冪等性・再送はM下限。", "外部API共通基盤とキューを別項目として加算しWebhook固有部分に限定する。"),
    "email-integration": assessed("S", 40000, "お知らせ配信", "テンプレート送信・再送は既存お知らせ4万円と同水準。", "キューと通知管理を別項目として加算し、単一メールサービス接続に限定する。"),
    "sms-integration": assessed("S", 50000, "プッシュ通知", "単一SMSサービス接続はS中央値。", "外部API共通基盤とキューを別項目として加算し、SMS固有処理に限定する。"),
    ...
    "maps-integration": assessed("M", 80000, "位置情報連携", "住所検索・座標取得のAPI部分は位置情報25万円より軽い。", "位置情報画面と外部API共通基盤を別項目として加算する。"),
    ...
    staging: assessed("S", 40000, "インフラ構築", "本番環境に追加する補助環境としてS帯。", "本番公開基盤を別項目として加算し、追加設定分だけを計上する。", { pricingClass: "foundation" }),
    cicd: assessed("M", 70000, "インフラ構築", "ビルド・検査・配備自動化は公開料金の8〜15万円帯。", "検証環境と単体テストを別項目として加算し、パイプライン構築に限定する。", { pricingClass: "foundation" }),
    ...
    cdn: assessed("S", 40000, "インフラ構築", "単純CDN設定はS帯。", "公開環境と監視を別項目として加算し、キャッシュ方針の追加分だけを評価する。", { pricingClass: "foundation" }),
    "queue-worker": assessed("S", 60000, "インフラ構築", "多数機能から共用される非同期基盤のためS上限。", "個別ジョブ処理は各機能側へ置き、再試行基盤だけを一度計上する。", { pricingClass: "foundation" }),
    "scalable-infra": assessed("L", 180000, "インフラ構築", "負荷分散・拡張設計・検証を含むためL下限。", "CDN・キュー・監視を別項目として加算し、拡張構成固有の作業を評価する。"),
    "disaster-recovery": assessed("L", 180000, "インフラ構築", "RTO/RPO・切替・復旧訓練を含むためL下限。", "バックアップと検証環境を別項目として加算し、復旧計画・訓練を対象にする。"),
    ...
    "performance-tests": assessed("M", 80000, "テスト・QA", "専用シナリオ・負荷投入・分析が必要なM帯。", "拡張インフラを別項目として加算し、性能検証工程だけを対象にする。"),
    "accessibility-tests": assessed("S", 30000, "アクセシビリティ対応", "実装対応7万円とは別の確認工程としてS下限。", "読み上げ・キーボード・コントラスト試験だけを計上する。"),
    "security-tests": assessed("M", 80000, "セキュリティ対策", "権限・入力・ライブラリ・設定試験はM帯。", "基本設計を別項目として加算し、検査と報告に限定する。"),
    "uat-support": assessed("M", 70000, "テスト・QA", "商談相手向けシナリオ・説明・指摘管理はM下限。", "実試験と環境を別項目として加算し、受入支援に限定する。"),
  ...
  const plainNames = {
    ...
    "wireframes": "主要画面のレイアウト案作成", "design-system": "画面デザインの共通ルール整備",
    "responsive-shell": "パソコン・スマートフォン両対応", "list-detail-ui": "データの一覧表示と詳細確認",
    "form-ui": "データの入力・修正画面", "advanced-search-ui": "条件を指定したデータ検索",
    "dashboard-ui": "業務状況の一覧表示画面", "charts-ui": "数値のグラフ表示",
    "calendar-ui": "予定・期限のカレンダー表示", "rich-editor-ui": "画像・表を含む文書の作成",
    "customer-portal-ui": "取引先向けの専用画面", "admin-ui": "運営者向けの設定・管理画面",
    "accessibility-ui": "高齢者・障がいのある方への画面配慮", "i18n-ui": "日本語以外の言語表示",
    "theme-ui": "会社別のロゴ・配色の切替", "api-foundation": "画面と保存データをつなぐ共通処理",
    ...
    "crm": "顧客・案件・対応履歴の一元管理", "report-generator": "登録内容からの報告書作成",
    "pdf-generator": "見積書・報告書のPDF化", "scheduled-jobs": "決まった時刻の自動通知・集計",
    "import-export": "Excel・CSVの取り込みと書き出し", "db-foundation": "入力データの保存先の構築",
    "schema-design": "保存する項目と関係の設計", "master-data": "分類・単価・選択肢の変更機能",
    "history-versioning": "変更前の内容と変更者の記録", "file-storage": "写真・文書の安全な保管",
    "search-index": "大量の文書からの高速検索", "analytics-store": "集計・分析用データの蓄積",
    "backup-restore": "データの定期保存と復元", "data-migration": "現在お使いのデータの移行",
    "security-baseline": "安全に使うための基本対策", "user-accounts": "利用者の登録・停止の管理",
    "session-security": "ログイン状態の安全な保持", "email-login": "メールアドレスでのログイン",
    "social-login": "Googleアカウント等でのログイン", "mfa": "確認コードによる本人確認の強化",
    "role-permissions": "役割ごとの閲覧範囲の設定", "organization-tenancy": "会社ごとのデータ分離",
    "external-portal-access": "社外の方への限定公開", "audit-log": "重要な操作の記録保存",
    ...
    "pwa": "ホーム画面からのアプリ起動", "offline-sync": "通信がない場所での入力と後送信",
    "native-app": "アプリストアでの配布", "push-notification": "スマートフォンへの通知",
    "geolocation": "現在地・住所・地図の利用", "external-api": "他サービスと連携するための基盤",
    "webhooks": "他サービスの更新の即時受信", "email-integration": "メールの自動送信",
    "sms-integration": "SMSの自動送信", "messaging-integration": "普段お使いのチャットへの通知",
    "payment-integration": "クレジットカード等での支払い", "accounting-integration": "会計ソフトへの請求・入金連携",
    ...
    "cdn": "写真・ファイルの高速表示", "queue-worker": "時間のかかる処理の順次実行",
    "scalable-infra": "利用者増加への対応", "disaster-recovery": "障害発生時の復旧体制",
    "unit-tests": "計算・入力チェックの自動確認", "integration-tests": "画面から保存までの連携確認",
    ...
    "project-management": "進捗・課題・変更の管理", "documentation-training": "操作マニュアルと説明会",
    "release-management": "公開版と切戻し手順の管理", "support-operation": "公開後の問い合わせ・障害対応",
  };
  ...
  const featureHierarchy = [
    ...
    { id: "backend", name: "業務処理・計算の自動化", groups: [
      ...
      { id: "backend-transaction", name: "予約・販売・顧客対応の管理", features: ["estimate-invoice", "booking-flow", "product-catalog", "inventory", "ecommerce", "subscription", "crm"] },
